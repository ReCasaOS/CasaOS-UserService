...
Everything else under `/v1/users`, and all of `/v2/users`, sits behind JWT middleware. It skips the check only for this box's other services: a request from `127.0.0.1` or `::1` that also carries `Authorization: Internal <secret>`, the secret the gateway writes to `/var/run/casaos/internal.secret` at every boot (see "Loopback is not an identity" below). Loopback alone gets no pass. Login, token refresh and the username list are open by necessity. The `/v2/users` API has nothing to do with accounts: it lists and deletes the events shown in the dashboard's notification list. The events themselves arrive over a websocket subscription to CasaOS-MessageBus (`route/event_listen.go`), not through this API.
...
- **Loopback is not an identity** (v0.4.25). Any request from 127.0.0.1 skipped the token. Loopback is not this box's services alone: a container on the host network, or any local account, reaches the same addresses. A request skips the token only when it is one of this box's services, which come from loopback with the secret the gateway writes for this boot (`/var/run/casaos/internal.secret`, readable by root only); the dashboard always had a token. A local process could change the administrator's password or switch the second factor off. The public routes (status, login, register, refresh, the second-factor check) are unchanged.
- **A session outlives a restart** (v0.4.27). The key tokens are signed with was generated at every start and kept in memory only, so a reboot, an upgrade, or a backup of the box that stops this service for the copy signed everybody out. It lives in `/var/lib/casaos/db/user-service.key` now, readable by root only, under the directory the box backup already carries. The key for the window between a password and a second factor is still made at start.
