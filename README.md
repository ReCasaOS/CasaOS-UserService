...
- **Module path** (this distribution). The module is `github.com/inkly/CasaOS-UserService` and depends on `github.com/inkly/CasaOS-Common v0.4.22`, so the import paths naming IceWhale in every log line, stack trace and `go version -m` on the shipped binary now name this distribution instead.
